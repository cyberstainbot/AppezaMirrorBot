...
def update_all_messages():
    ...
    with download_dict_lock:
        ...
        msg += f"\n<b>ꜰʀᴇᴇ :</b> <code>{free}</code> | <b>ᴜᴘᴛɪᴍᴇ :</b> <code>{currentTime}</code>\n<b>ᴅʟ :</b> <code>{dlspeed}/s</code> 🔻 | <b>ᴜʟ :</b> <code>{ulspeed}/s</code> 🔺\n"
    with status_reply_dict_lock:
...
def sendStatusMessage(msg, bot):
    ...
    if progress is None:
        progress, buttons = get_readable_message()
    progress += f"<b>ᴄᴘᴜ:</b> <code>{psutil.cpu_percent()}%</code>" \
                f" <b>ʀᴀᴍ:</b> <code>{psutil.virtual_memory().percent}%</code>" \
                f" <b>ᴅɪꜱᴋ:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        dlspeed_bytes = 0
